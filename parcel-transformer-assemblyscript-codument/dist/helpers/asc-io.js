...
exports.ascIO = {
    ...
    write: (compilationArtifacts, filename, contents, baseDir) => {
        ...
        contents &&
            console.log(`${PREF_WRITE} ${path} `.padEnd(DOT_PADDING, ".") +
        ...
        switch (true) {
            case filename.endsWith(artifact_file_type_1.ArtifactFileType.MAP):
                compilationArtifacts[artifact_file_type_1.ArtifactFileType.MAP] = contents;
                break;
            case filename.endsWith(artifact_file_type_1.ArtifactFileType.WAT):
                compilationArtifacts[artifact_file_type_1.ArtifactFileType.WAT] = contents;
                break;
            case filename.endsWith(artifact_file_type_1.ArtifactFileType.D_TS):
                compilationArtifacts[artifact_file_type_1.ArtifactFileType.D_TS] = contents;
                break;
            case filename.endsWith(artifact_file_type_1.ArtifactFileType.WASM):
                compilationArtifacts[artifact_file_type_1.ArtifactFileType.WASM] = contents;
                break;
            case filename.endsWith(artifact_file_type_1.ArtifactFileType.JS):
                compilationArtifacts[artifact_file_type_1.ArtifactFileType.JS] = contents;
                break;
            default:
    ...
    read: (inputCode, filename, baseDir) => {
        ...
        try {
            ...
            if (isConfigFile && asconfigCache) {
                console.log(`${PREF_READ} ${absolutePath} `.padEnd(DOT_PADDING, ".") +
            ...
            if (isConfigFile) {
                ...
                console.log(`[ASC] 📦 Cached ${absolutePath} `.padEnd(DOT_PADDING, ".") +
                    ` ${content.length} bytes`);
            }
            return content;
            //
...
//# sourceMappingURL=asc-io.js.map